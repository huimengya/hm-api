...
import com.qingshu.hmapibackend.mapper.UserMapper;
import com.qingshu.hmapibackend.model.dto.user.*;
import com.qingshu.hmapibackend.model.entity.User;
import com.qingshu.hmapibackend.model.enums.UserAccountStatusEnum;
import com.qingshu.hmapibackend.model.vo.UserVO;
import com.qingshu.hmapibackend.service.UserService;
import com.qingshu.hmapibackend.utils.RedissonLockUtil;
...
import static com.qingshu.hmapibackend.constant.EmailConstant.CAPTCHA_CACHE_KEY;
import static com.qingshu.hmapibackend.constant.UserConstant.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public long userEmailRegister(UserEmailRegisterRequest userEmailRegisterRequest) {
        ...
        return redissonLockUtil.redissonDistributedLocks(redissonLock, () -> {
            ...
            user.setUserAvatar("https://qnm.hunliji.com/o_1gjo5pg1uf9a161f8410i6v2n8n.jpg");
            user.setUserName(userName);
            user.setAccessKey(accessKey);
    ...
    public UserVO userEmailLogin(UserEmailLoginRequest userEmailLoginRequest, HttpServletRequest request) {
        ...
        // 用户不存在
        if (user == null) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "该邮箱未注册");
        }
    ...
    public UserVO getLoginUser(HttpServletRequest request) {
        ...
        // 拼接用户头像地址
        user.setUserAvatar(FileConstant.COS_HOST + user.getUserAvatar());
        UserVO userVO = new UserVO();
        BeanUtils.copyProperties(user, userVO);
        return userVO;
    }
    ...
    public boolean isAdmin(HttpServletRequest request) {
        ...
        long userId = currentUser.getId();
        User user = this.getById(userId);
        // 判断是否为管理员 返回true:是管理员 false:不是管理员
    ...
    public void validUser(User user, boolean add) {
        ...
        //  5. 账户不包含特殊字符
        // 匹配由数字、小写字母、大写字母组成的字符串也可以是邮箱,且字符串的长度至少为1个字符
        String pattern = "^(?=.*[0-9a-zA-Z@]).+$";
